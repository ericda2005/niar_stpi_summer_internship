...
def get_techtimes_categories_and_links(homepage_url="https://www.techtimes.com/"):
    ...
    # 1. 抓取有明確板塊標題的文章
    block_titles = soup.find_all('div', class_=lambda x: x and 'block-title' in x)
    for title_div in block_titles:
        category_name = title_div.get_text(strip=True)
        if not category_name:
            continue
            
        if category_name not in raw_category_dict:
            raw_category_dict[category_name] = []
            
        parent_container = title_div.parent
        articles = parent_container.find_all('article')
        
        for article in articles:
            heading_tag = article.find(['h2', 'h3', 'h4', 'h5', 'h6'])
            if heading_tag:
                ...
                if a_tag:
                    ...
                    if not href.startswith('http'):
                        href = "https://www.techtimes.com" + href
                    
                    if href not in seen_urls:
                        seen_urls.add(href)
                        raw_category_dict[category_name].append({
                            '標題': title, 
                            'url': href,
                            '分類': category_name
                        })
    ...
    final_category_dict = {}
    if uncategorized:
        final_category_dict["頂部區塊 (未分類)"] = uncategorized
    for cat, articles in raw_category_dict.items():
        if articles:
            final_category_dict[cat] = articles

    return final_category_dict


def scrape_techtimes_with_refs(main_url, status_text=None, current_article_info=""):
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36'}
    try:
    ...
    article_body = soup.find('div', class_='content article-body')
    if not article_body:
        return []
    ...
    for a in article_body.find_all('a', href=True):
        ...
        if href.startswith('/'):
            href = "https://www.techtimes.com" + href
        if href.startswith('http') and href not in ref_urls:
...
with col2:
    if st.button("搜尋", type="primary", use_container_width=True):
        with st.spinner("正在掃描首頁板塊與文章..."):
            cat_dict = get_techtimes_categories_and_links(url_input)
